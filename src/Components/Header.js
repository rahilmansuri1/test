import AddProduct from "./ProductModel";
...
const Header = ({ setRefreshKey }) => {
    ...
    const getUser = (userId) => {
        fetch(`http://localhost:8080/user/${userId}`).then(
            (result) => {
    ...
    return (
        <header className="header sticky top-0 bg-gray-900 text-white shadow-md flex items-center justify-between px-24 py-02" >
            <img class="w-10 h-10 rounded-full" src="https://cdn.pixabay.com/photo/2013/07/13/10/07/man-156584__480.png" alt="Rounded avatar" />
            <h1 className="w-3/12">
                {user.fullName}
            </h1>
            <nav className="nav font-semibold text-lg">
                <ul className="flex items-center">
                    <li className="p-4 border-b-2 border-green-500 border-opacity-0 hover:border-opacity-100 hover:text-green-500 duration-200">
                        <AddProduct setRefreshKey={setRefreshKey} />
                    </li>
